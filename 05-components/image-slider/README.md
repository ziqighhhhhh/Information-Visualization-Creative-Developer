# image-slider — 纯 CSS 图片滑块

可复用组件：radio hack 驱动的无 JS 滑块。演示见 `index.html`。

## 来源（第三方收集）

- 原作：Flight slider - HTML+CSS by Alvaro Montoro
- 链接：https://codepen.io/alvaromontoro/pen/oNVEqgz
- 许可：MIT (CodePen public pen default)
- 说明：自动收集的参考实现，归档时未逐行审计；运行异常先查控制台与网络（CDN）

## 依赖

无依赖

## 变体（demos/）

- `demos/fancy.html`：Fancy Slider — Nikolay Talanov（https://codepen.io/suez/pen/wMMgXp）

## 归档

2026-09 由 inbox 审查后分类入 `05-components/`。
